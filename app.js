document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================================================
  // 5. Select Room Trigger -> Booking Form Autofill
  // ==========================================================================
  const selectRoomBtns = document.querySelectorAll('.select-room-btn');
  const roomSelectDropdown = document.getElementById('room-type');

  selectRoomBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      const selectedRoom = btn.getAttribute('data-room-type');
      if (roomSelectDropdown) {
        roomSelectDropdown.value = selectedRoom;
      }
    });
  });

  // Set default check-in/out dates (today & tomorrow)
  ...
  // ==========================================================================
  // 6. Booking Form -> Construct & Send WhatsApp Message
  // ==========================================================================
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', (e) => {
      ...
      const comments = document.getElementById('special-requests').value;

      // Map value to friendly text
      const roomTypesMap = {
        'deluxe': 'Deluxe Room (₹2,499/night)',
        'super-deluxe': 'Super Deluxe Room (₹3,499/night)',
        'family-suite': 'Family Suite (₹4,999/night)'
      };
      
      const roomText = roomTypesMap[roomVal] || roomVal;

      // Construct message content
      const msg = `*New Booking Inquiry - Hotel Srijan Vrindavan*\n` +
                  `---------------------------------------------\n` +
                  `*Name:* ${name}\n` +
                  `*Email:* ${email}\n` +
                  `*Phone/WhatsApp:* ${phone}\n` +
                  `*Room Category:* ${roomText}\n` +
                  `*Total Guests:* ${guests}\n` +
                  `*Check-In:* ${checkin}\n` +
                  `*Check-Out:* ${checkout}\n` +
                  (comments ? `*Special Request:* ${comments}\n` : '') +
                  `---------------------------------------------\n` +
                  `Please check availability and send confirmation. Thank you!`;

      // Hotel Business Number (simulated for project template, can be modified)
      const hotelPhone = '919876543210';
      const encodedMsg = encodeURIComponent(msg);
      
      // WhatsApp API link
      const waLink = `https://wa.me/${hotelPhone}?text=${encodedMsg}`;
      
      // Open in a new tab
      window.open(waLink, '_blank');
      
      // Optional: Inform user
      alert("Redirecting to WhatsApp to send your booking inquiry to Hotel Srijan...");
    });
  }
